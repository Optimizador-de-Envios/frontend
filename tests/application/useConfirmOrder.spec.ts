...
describe('useConfirmOrder (HU-05)', () => {
  ...
  it('should set selectedOption in store when confirm succeeds', async () => {
    vi.mocked(confirmOrder).mockResolvedValueOnce(undefined)
    const { result } = renderHook(() => useConfirmOrder())
    await act(async () => {
      await result.current.confirm(validOrder, selectedOption)
    })
  ...
  it('should NOT store selectedOption in store when confirm fails', async () => {
    vi.mocked(confirmOrder).mockRejectedValueOnce(new Error('Network error'))
    const { result } = renderHook(() => useConfirmOrder())
    await act(async () => {
      await result.current.confirm(validOrder, selectedOption)
